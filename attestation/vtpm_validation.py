...
class VtpmValidation:
    ...
    @staticmethod
    def check_nonce_match(expected_nonce: str, token_nonces: list[str]) -> bool:
        """
        Check if any of the token nonces match the expected nonce or its prefix.
        
        This handles cases where the attestation service might modify the nonce
        but preserve its prefix.
        
        Args:
            expected_nonce: The nonce we expect to find
            token_nonces: List of nonces from the token
            
        Returns:
            bool: True if a match is found, False otherwise
        """
        # Direct match
        if expected_nonce in token_nonces:
            return True
            
        # Check for prefix match if the expected nonce has a prefix format
        if "-" in expected_nonce:
            prefix = expected_nonce.split("-")[0]
            for nonce in token_nonces:
                if nonce.startswith(f"{prefix}-"):
                    return True
