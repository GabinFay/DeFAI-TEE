...
def get_simulated_token() -> str:
    ...
    token_path = Path(__file__).parent / "simulated_token.txt"
    ...
    try:
        ...
        # Check if we need to update the nonce
        if "nonces" not in decoded:
            # Add our custom nonce format
            decoded["nonces"] = ["flare-hackathon-12345678"]
            
            # Re-encode the token
            # Using a dummy key for encoding
            dummy_key = "dummy_key_for_encoding"
            token = jwt.encode(decoded, dummy_key, algorithm="HS256")
    except Exception as e:
        logger.error(f"Error processing simulated token: {e}")
